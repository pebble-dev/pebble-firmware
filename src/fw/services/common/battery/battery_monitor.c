...
void battery_monitor_handle_state_change_event(PreciseBatteryChargeState state) {
  ...
  bool critical = (state.charge_percent == 0) && !state.is_charging && !state.is_plugged;

#ifndef RECOVERY_FW
  const uint32_t LOW_POWER_PERCENT = ratio32_from_percent(BOARD_CONFIG_POWER.low_power_threshold);

  bool low_power = !state.is_charging && !state.is_plugged && (state.charge_percent <= LOW_POWER_PERCENT);
